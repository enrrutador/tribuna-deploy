...
export default function Tournament() {
  ...
  return (
    <div className="space-y-4">
      {/* Tournament banner — compact horizontal bar */}
        ...
        <div className="flex items-center gap-3 px-4 z-10">
          ...
          <div>
            ...
            <div className="text-gray-400 text-[11px] capitalize">
              {tournament.category === "argentina" ? "Argentina" : tournament.category === "destacados" ? "Destacado" : "Internacional"}
            </div>
          </div>
        </div>
      ...
      {/* Tabs — same style as match filter tabs */}
              ...
              <table className="w-full text-[13px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {standingsData.standings.map((row, idx) => (
                    <tr
                      key={row.team.id}
                      className={`hover:bg-[#f8faff] transition-colors ${idx < 4 ? "border-l-2 border-l-[#1a9be6]" : idx < 6 ? "border-l-2 border-l-amber-400" : "border-l-2 border-l-transparent"}`}
                    >
              ...
              <table className="w-full text-[13px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {scorersData.scorers.map((entry, idx) => (
                    <tr key={entry.player.id} className="hover:bg-[#f8faff] transition-colors">
                      ...
                      <td className="px-3 py-3 hidden sm:table-cell">
                        <span className="text-gray-500">{entry.team.name}</span>
                      </td>
